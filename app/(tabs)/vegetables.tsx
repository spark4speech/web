...
import { vegetables } from "@/constants/assets/vegetables";
    ...
    <View style={styles.container}>
      ...
      <View style={styles.orangeBar}>
        ...
        <TouchableOpacity onPress={handleSpeakSentence}>
          <Image  
            ...
            style={{ width: 160, height: 160, marginBottom: -3, marginRight: -62  }}
          />
        </TouchableOpacity>
      ...
      <ScrollView contentContainerStyle={styles.gridContainer}>
        {Array.from({ length: 72 }).map((_, index) => {
          const vegetable = vegetables[Math.floor(index / 2)];
          return (
            <View key={index} style={styles.gridItem}>
              {index % 2 === 0 && index % 24 < 12 && vegetable ? (
                <TouchableOpacity onPress={() => handlePress(vegetable.name)}>
                  <Image source={vegetable.url} style={styles.icon} />
                  <Text style={styles.iconText}>{vegetable.name}</Text>
                </TouchableOpacity>
              ) : index % 2 === 1 && index % 24 > 11 && vegetable ? (
                <TouchableOpacity onPress={() => handlePress(vegetable.name)}>
                  <Image source={vegetable.url} style={styles.icon} />
                  <Text style={styles.iconText}>{vegetable.name}</Text>
                </TouchableOpacity>
                ...
                <Image
                  source={{
                    uri: vegetable?.name == "" ? vegetable?.url || "" : "",
                  }}
                  style={styles.icon}
                />
              )}
