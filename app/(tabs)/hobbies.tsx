    ...
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.gridContainer}>
        {Array.from({ length: 72 }).map((_, index) => {
          const hobby = hobbies[Math.floor(index / 2)];
          return (
            <View key={index} style={styles.gridItem}>
              {index % 2 === 0 && index % 24 < 12 && hobby ? (
                <TouchableOpacity onPress={() => handlePress(hobby.name)}>
                  <Image source={hobby.url} style={styles.icon} />
                  <Text style={styles.iconText}>{hobby.name}</Text>
                </TouchableOpacity>
              ) : index % 2 === 1 && index % 24 > 11 && hobby ? (
                <TouchableOpacity onPress={() => handlePress(hobby.name)}>
                  <Image source={hobby.url} style={styles.icon} />
                  <Text style={styles.iconText}>{hobby.name}</Text>
                </TouchableOpacity>
