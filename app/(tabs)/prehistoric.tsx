...
import { prehistoric } from "@/constants/assets/prehistoric";
...
const AnimalsScreen = () => {
  ...
  const handlePress = (prehistoricName: string) => {
    setSentence((prev) => (prev ? `${prev} ${prehistoricName}` : prehistoricName));

    Speech.speak(prehistoricName);
  };
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Grid of prehistoric animals */}
      <ScrollView contentContainerStyle={styles.gridContainer}>
        {Array.from({ length: 72 }).map((_, index) => {
          const p = prehistoric[Math.floor(index / 2)];
          return (
