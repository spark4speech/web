...
const ColorScreen = () => {
  ...
  const handlePress = (colorName: string) => {
    setSentence((prev) => (prev ? `${prev} ${colorName}` : colorName));
        ...
        {Array.from({ length: 72 }).map((_, index) => {
          ...
          return (
            <View key={index} style={styles.gridItem}>
              {index % 2 === 0 && index % 24 < 12 && color ? (
                <TouchableOpacity onPress={() => handlePress(color.name)}>
                  <Image source={color.url} style={styles.icon} />
                  <Text style={styles.iconText}>{color.name}</Text>
                </TouchableOpacity>
              ) : index % 2 === 1 && index % 24 > 11 && color ? (
                <TouchableOpacity onPress={() => handlePress(color.name)}>
                  <Image source={color.url} style={styles.icon} />
                  <Text style={styles.iconText}>{color.name}</Text>
                </TouchableOpacity>
