...
import { actions } from "@/constants/assets/actions";
    ...
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.gridContainer}>
        {Array.from({ length: 72 }).map((_, index) => {
          const action = actions[Math.floor(index / 2)];
          return (
            <View key={index} style={styles.gridItem}>
              {index % 2 === 0 && index % 24 < 12 && action ? (
                <TouchableOpacity onPress={() => handlePress(action.name)}>
                  <Image source={action.url} style={styles.icon} />
                  <Text style={styles.iconText}>{action.name}</Text>
                </TouchableOpacity>
              ) : index % 2 === 1 && index % 24 > 11 && action ? (
                <TouchableOpacity onPress={() => handlePress(action.name)}>
                  <Image source={action.url} style={styles.icon} />
                  <Text style={styles.iconText}>{action.name}</Text>
                </TouchableOpacity>
