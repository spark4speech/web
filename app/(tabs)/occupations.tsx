    ...
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.gridContainer}>
        {Array.from({ length: 72 }).map((_, index) => {
          const occupation = occupations[Math.floor(index / 2)];
          return (
            <View key={index} style={styles.gridItem}>
              {index % 2 === 0 && index % 24 < 12 && occupation ? (
                <TouchableOpacity onPress={() => handlePress(occupation.name)}>
                  <Image source={occupation.url} style={styles.icon} />
                  <Text style={styles.iconText}>{occupation.name}</Text>
                </TouchableOpacity>
              ) : index % 2 === 1 && index % 24 > 11 && occupation ? (
                <TouchableOpacity onPress={() => handlePress(occupation.name)}>
                  <Image source={occupation.url} style={styles.icon} />
                  <Text style={styles.iconText}>{occupation.name}</Text>
                </TouchableOpacity>
                ...
                <Image
                  source={{
                    uri: occupation?.name == "" ? occupation?.url || "" : "",
                  }}
                  style={styles.icon}
                />
              )}
