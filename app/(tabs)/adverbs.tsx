...
import { adverbs } from "@/constants/assets/adverbs";
    ...
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.gridContainer}>
        {Array.from({ length: 72 }).map((_, index) => {
          const adverb = adverbs[Math.floor(index / 2)];
          return (
            <View key={index} style={styles.gridItem}>
              {index % 2 === 0 && index % 24 < 12 && adverb ? (
                <TouchableOpacity onPress={() => handlePress(adverb.name)}>
                  <Image source={adverb.url} style={styles.icon} />
                  <Text style={styles.iconText}>{adverb.name}</Text>
                </TouchableOpacity>
              ) : index % 2 === 1 && index % 24 > 11 && adverb ? (
                <TouchableOpacity onPress={() => handlePress(adverb.name)}>
                  <Image source={adverb.url} style={styles.icon} />
                  <Text style={styles.iconText}>{adverb.name}</Text>
                </TouchableOpacity>
