    ...
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.gridContainer}>
        {Array.from({ length: 72 }).map((_, index) => {
          const holiday = holidays[Math.floor(index / 2)];
          return (
            <View key={index} style={styles.gridItem}>
              {index % 2 === 0 && index % 24 < 12 && holiday ? (
                <TouchableOpacity onPress={() => handlePress(holiday.name)}>
                  <Image source={holiday.url} style={styles.icon} />
                  <Text style={styles.iconText}>{holiday.name}</Text>
                </TouchableOpacity>
              ) : index % 2 === 1 && index % 24 > 11 && holiday ? (
                <TouchableOpacity onPress={() => handlePress(holiday.name)}>
                  <Image source={holiday.url} style={styles.icon} />
                  <Text style={styles.iconText}>{holiday.name}</Text>
                </TouchableOpacity>
