...
import { time } from "@/constants/assets/time";
    ...
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.gridContainer}>
        {Array.from({ length: 72 }).map((_, index) => {
          const timeob = time[Math.floor(index / 2)];
          return (
            <View key={index} style={styles.gridItem}>
              {index % 2 === 0 && index % 24 < 12 && timeob ? (
                <TouchableOpacity onPress={() => handlePress(timeob.name)}>
                  <Image source={timeob.url} style={styles.icon} />
                  <Text style={styles.iconText}>{timeob.name}</Text>
                </TouchableOpacity>
              ) : index % 2 === 1 && index % 24 > 11 && timeob ? (
                <TouchableOpacity onPress={() => handlePress(timeob.name)}>
                  <Image source={timeob.url} style={styles.icon} />
                  <Text style={styles.iconText}>{timeob.name}</Text>
                </TouchableOpacity>
